...
nbrDensities    = length(densities);
newAngles       = zeros(1, N);
newAgentVel     = zeros(2, N);
avgW            = zeros(2, nbrAverage);
finalW          = zeros(2, nbrDensities);
avgPolarization = zeros(1, nbrAverage);
polarization    = zeros(1, nbrDensities);
cost            = zeros(1, N);
benefit         = zeros(1, N);
fitness         = zeros(1, N);
...
%             drawnow;
        ...
        avgPolarization(i_avg)  = GetPolarization( agentVel, s );
        avgW(:, i_avg)          = [meanW_a; meanW_m];
...
save('densityRun', 'densities', 'polarization', 'finalW', 'N', 'nbrAverage', 'repulsionRadius', 's', 'sightRadius',...
